...
export default function KanbanPage() {
  ...
  const [newCard, setNewCard] = useState({ titulo: '', status: 'pendente', prioridade: 'med', responsavel_id: '' });
  ...
  const handleCreateCard = async () => {
    ...
    setSaving(false);
    setShowModal(false);
    setNewCard({ titulo: '', status: 'pendente', prioridade: 'med', responsavel_id: '' });
  };
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <div className={styles.breadcrumb}>
            {session.empresa?.nome} › {session.squad?.nome} › {session.usuario?.nome}
          </div>
        </div>
        <div className={styles.headerRight}>
          <button
            className={`${styles.filterBtn} ${meusFiltro ? styles.filterActive : ''}`}
            onClick={() => setMeusFiltro(v => !v)}
          >
            Meus cards
          </button>
      ...
      <div className={styles.statsRow}>
        {COLS.map(col => (
          <div key={col.id} className={styles.stat}>
            <div className={styles.statVal} style={{ color: col.color }}>
      ...
      {loading ? (
        <div className={styles.loading}>Carregando cards...</div>
      ) : (
                      ...
                      >
                        ...
                        <div className={styles.cardFooter}>
                          ...
                          <span
                            ...
                            style={{ background: prioBg[card.prioridade], color: prioColor[card.prioridade] }}
                          >
                            {prioLabel[card.prioridade]}
                          </span>
                        </div>
                      </div>
                    );
        ...
        <div className={styles.modalBg} onClick={(e) => { if (e.target === e.currentTarget) setShowModal(false); }}>
          <div className={styles.modal}>
            ...
            <input
              ...
              onChange={e => setNewCard(p => ({ ...p, titulo: e.target.value }))}
              autoFocus
            />
